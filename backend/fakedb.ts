...
export const editPost = (post: any) => {
  ...
  let postToBeEditted = posts.filter((db_post)=> db_post.id===post.id);
  console.log(postToBeEditted);
  postToBeEditted[0].title=post.title;
  postToBeEditted[0].category=post.category;
  postToBeEditted[0].content=post.content;
  postToBeEditted[0].image=post.image;
};
